        ...
        github.get "/orgs/RHDHR/teams", (teams) ->
          ...
          for team in teams
            if team.name == msg.match[2]
              flag = 1
              team_id = team.id
              data = { name: msg.match[1], description: "auto generated by kuma", private : true, has_issues : true, has_wiki : true, has_downloads : true, team_id : team_id}
              github.post "/orgs/RHDHR/repos", data, (repo) ->
                if repo.full_name != ""
                  msg.send "Repository #{repo.full_name} successfully created. Access here #{repo.html_url}"
          if flag == 0
            msg.send "no such team name"
